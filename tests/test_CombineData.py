...
def test_store_data(combine_data):
    combine_data.store_data('out/test.csv')
    with open('out/test.csv', newline='',encoding='utf-8') as csvfile:
...
def test_file_name_generator(combine_data):
    assert re.match("^out/2022-11-24--23-", combine_data.file_name_generator())
